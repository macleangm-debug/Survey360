...
const statusConfig = {
  ...
  closed: { label: 'Closed', color: 'bg-gray-500/10 text-gray-500', icon: XCircle },
};

const CaseRow = ({ caseItem, onStatusChange, onView }) => {
  const status = statusConfig[caseItem.status] || statusConfig.open;
  const StatusIcon = status.icon;

  return (
    <TableRow className="cursor-pointer hover:bg-accent/50" onClick={() => onView(caseItem)}>
      <TableCell className="font-mono text-sm">{caseItem.respondent_id}</TableCell>
      ...
      <TableCell>{caseItem.visit_count}</TableCell>
      <TableCell>{caseItem.last_visit ? formatDate(caseItem.last_visit) : 'Never'}</TableCell>
...
export function CasesPage() {
  ...
  const handleViewCase = (caseItem) => {
    // For now, just show a toast. In future, navigate to case detail page
    toast.info(`Case ${caseItem.respondent_id}: ${caseItem.visit_count} visits`);
  };
